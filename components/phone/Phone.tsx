    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 border-b border-white/[0.06] px-3 py-2 shrink-0">
        <div className="h-7 w-7 rounded-full bg-blue-500/20 border border-blue-500/30 flex items-center justify-center">
          {/* Speech bubble = SMS/Messages icon */}
          ...
          {smsMsgs.map((msg, i) =>
            visible.includes(i) ? (
              <motion.div
                ...
                className="bg-blue-600/15 border border-blue-500/25 rounded-2xl rounded-tl-sm px-3 py-2 max-w-[90%]"
              >
                <p className="text-blue-400 text-[8px] font-semibold mb-1">{msg.from}</p>
                {msg.lines.map((l, j) => (
      ...
      <div className="border-t border-white/[0.06] px-3 py-2 flex gap-2 items-center shrink-0">
        <div className="flex-1 bg-zinc-800/60 rounded-full px-3 py-1.5">
          <p className="text-zinc-600 text-[10px]">Text message</p>
        </div>
